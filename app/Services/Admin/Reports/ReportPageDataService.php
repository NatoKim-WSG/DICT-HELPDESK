...
class ReportPageDataService
{
    ...
    public function buildIndexViewData(Request $request, User $user): array
    {
        ...
        [$monthlyReportRows, $monthlyGraphPoints] = $this->monthlyDataset(clone $scopedTickets);
        $selectedMonthContext = $this->resolveSelectedMonthContext($request, $monthlyReportRows, clone $scopedTickets);
        ...
        $detailScope = $this->resolveDetailScope($request, $monthlyReportRows, $selectedMonthKey);
        ...
        $dailyScope = $this->resolveDailyScope(
            $request,
            $monthlyReportRows,
            $detailFilterApplied,
            $detailMonthKey,
        ...
        $totalTicketsThisPeriod = $selectedMonthIsAllTime
            ? $this->ticketCount(clone $scopedTickets)
            : (int) ($selectedMonthRow['received'] ?? 0);
        $backlogThisPeriod = $this->openTicketCountAtCutoff(clone $scopedTickets, $selectedPeriodEnd);
        ...
        $mixScopeLabel = $mixBreakdownData['mixScopeLabel'];
        ...
        $detailOverview = $this->buildDetailOverview(
            clone $scopedTickets,
            $detailScopeStart,
            $detailScopeEnd,
            $detailScopeLabel,
        ...
        if ($detailFilterApplied) {
            ...
            [, $monthlyPerformanceGraphPoints] = $this->monthlyDataset(clone $monthlyPerformanceScopedTickets);
        }
        ...
        $ticketTrend = $this->buildTicketTrend(clone $scopedTickets);
    ...
    {
        $scopedTickets = $this->scopedTicketQueryForUser($user);
        [$monthlyReportRows] = $this->monthlyDataset(clone $scopedTickets);
    ...
    private function resolveSelectedMonthContext(Request $request, $monthlyReportRows, Builder $scopedTickets): array
    {
    ...
    private function resolveDetailScope(Request $request, $monthlyReportRows, string $selectedMonthKey): array
    {
    ...
    private function resolveDailyScope(
        Request $request,
        $monthlyReportRows,
    ...
    private function buildMixBreakdownData(
        Builder $scopedTickets,
        bool $detailFilterApplied,
        ...
        if ($detailFilterApplied) {
            ...
            $mixTotalCreated = $this->createdTicketCountForRange(clone $scopedTickets, $detailScopeStart, $detailScopeEnd);
        ...
        $mixTotalCreated = $this->ticketCount(clone $scopedTickets);
        $priorityCounts = $this->priorityCountsForScope(clone $scopedTickets);
        $categoryCounts = $this->categoryCountsForScope(clone $scopedTickets);
    ...
    private function buildDetailOverview(
        Builder $scopedTickets,
        Carbon $detailScopeStart,
        Carbon $detailScopeEnd,
        string $detailScopeLabel,
        ...
        $detailTotalCreated = $this->createdTicketCountForRange(clone $scopedTickets, $detailScopeStart, $detailScopeEnd);
        ...
        return [
            ...
            'backlog_end' => $this->openTicketCountAtCutoff(clone $scopedTickets, $detailScopeEnd),
        ];
    }

    private function buildTicketTrend(Builder $scopedTickets)
    {
    ...
    private function buildTopTechnicians(
        Builder $scopedTickets,
        bool $detailFilterApplied,
        Carbon $detailScopeStart,
        Carbon $detailScopeEnd
    ...
    private function monthlyDataset(Builder $scopedTickets): array
    {
    ...
    private function ticketCount(Builder $scopedTickets): int
    {
        $cacheKey = $this->queryScopeSignature($scopedTickets);
    ...
    private function createdTicketCountForRange(Builder $scopedTickets, Carbon $start, Carbon $end): int
    {
        $cacheKey = $this->queryScopeSignature($scopedTickets)
    ...
    private function openTicketCountAtCutoff(Builder $scopedTickets, Carbon $cutoff): int
    {
    ...
    private function statusCountsForScope(Builder $scopedTickets): Collection
    {
        $cacheKey = $this->queryScopeSignature($scopedTickets);
    ...
    private function priorityCountsForScope(Builder $scopedTickets): Collection
    {
        $cacheKey = $this->queryScopeSignature($scopedTickets);
    ...
    private function categoryCountsForScope(Builder $scopedTickets): Collection
    {
        $cacheKey = $this->queryScopeSignature($scopedTickets);
